...
const GenreSection = ({ data, dataNum, show, color }: Props) => {
  return (
    <WrapperSection>
      <TitleDiv>
        <HDiv>
          {show ? (
            <TitleLink to="section">인기가요 & 신곡</TitleLink>
          ) : (
            <TitleP color={color}>인기가요 & 신곡</TitleP>
          )}
        </HDiv>
      ...
      <GridDiv>
        {data
          ...
          .map((item: Item, idx: number) => (
            <GenreAlbumBox item={item} key={idx} />
          ))}
      </GridDiv>
    </WrapperSection>
  );
