...
import HomeAlbumBox from '../atoms/HomeAlbumBox';
...
const HomeSection = ({ data, dataNum, show, color }: Props) => {
  return (
    <WrapperSection>
      <TitleDiv>
        <HDiv>
          {show ? (
            <TitleLink to="section">{data.type}</TitleLink>
          ) : (
            <TitleP color={color}>{data.type}</TitleP>
          )}
        </HDiv>
      ...
      <GridDiv>
        {data.data
          ...
          .map((item: Item, idx: number) => (
            <HomeAlbumBox item={item} key={idx} />
          ))}
      </GridDiv>
    </WrapperSection>
  );
